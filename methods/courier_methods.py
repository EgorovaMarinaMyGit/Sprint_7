...
class CourierMethods:
    ...
    @allure.step("Создание курьера без логина/пароля")
    def courier_create_no_smth_data(self, params):
        response = requests.post(f'{BASE_URL}{COURIERS_URL}', data=params)
    ...
    @allure.step("Авторизация курьера с некорректным или незаполненным логином/паролем")
    def courier_login_incorrect_smth_data(self, params):
        response = requests.post(f'{BASE_URL}{COURIERS_URL}{LOGIN_URL}', data=params)
        return response.json(), response.status_code
